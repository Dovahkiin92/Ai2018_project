...
public class ApproximatedArchiveController {
            ...
            @AuthenticationPrincipal String username)
    {
        ...
        try {
            // get archives visible in map, in time interval, available for current user
            archives = archiveService.getArchivesWithinPolygonAndTime(
                    positionService.buildRect( req.getTopRight(),
                            req.getBottomLeft()),
                            req.getFrom(),
                            req.getTo(),
                            username
            ...
            app =  archives
                    .stream()
                    .map(ArchiveService::approximate).collect(Collectors.toList());
        }
        catch (Exception e){
